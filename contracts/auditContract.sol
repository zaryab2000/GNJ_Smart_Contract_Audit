...
// NEEDS TO BE CHECKED
  function findPercent(uint256 value) public view returns (uint256)  {
    ...
    uint256 percent = value.mul(basePercent).div(10000); // WHY DIVIDED BY 1000?
    return percent;
  }
...
// NEEDS TO BE CHECKED
  function transfer(address to, uint256 value) public returns (bool) {
   ...
   // _totalSupply = _totalSupply.sub(tokensToBurn); -> WHY COMMENTED?
...
// NEEDS TO BE CHECKED
  function multiTransfer(address[] memory receivers, uint256[] memory amounts) public {
    uint256 localVariable = receivers.length;
    for (uint256 i = 0; i < localVariable; i++) {
